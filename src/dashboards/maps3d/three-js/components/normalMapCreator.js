...
function reduceHeightMap( image, width ) {
    let out = [];
    let row = -1;
    for(let i=0; i < image.image.data.length; i+=4) { // Jimp uses RGBA format, so each pixel is 4 elements in the array
        if(i % width === 0) { // If you've cycled through one row, add the next
            out.push([]);
            row++;
        }
        out[row].push(image.image.data[i]); // grab the first element, which is the Red channel. should be equal to the green and blue channels.
    }
...
export function generateNormalMap( image ) {
    let heightMap = reduceHeightMap( getHeightMap( image ), image.width );
...
export default generateNormalMap;